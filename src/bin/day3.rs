...
fn trees_hit(slope: &[&str], step_x: i64, step_y: i64) -> i64 {
	let mut hit = 0;
	let mut x = 0;
	let mut y = 0;
	...
	while y < goal - 1 {
		x = (x + step_x) % width;
		y += step_y;
		// I don't like this usize bs again :(
		let pos = slope[y as usize].chars().collect::<Vec<_>>()[x as usize];
		if pos == '#' {
			hit += 1;
		}
	}
...
fn test2() {
	...
	assert_eq!(336, results.iter().fold(1, |acc, x| acc * x));
}

fn main() {
	let mut buf = String::new();
	io::stdin().read_to_string(&mut buf).unwrap();
	let lines = buf.lines().map(|f| f.trim()).collect::<Vec<_>>();
	...
	let hits = results.iter().fold(1, |acc, x| acc * x);
	println!("{}", hits);
}
